...
public class WebSecurityConfig {
   ...
   protected DefaultSecurityFilterChain securityFilterChain(
      ...
      http
         ...
         //redirectionEndpoint에 설정된 곳으로 리다이렉트하라는 뜻
         //아무 주소도 넣지 않았다면 baseUri인 "http://localhost:5000으로
         //리다이렉트한다.
         .and()
         .authorizationEndpoint()
         .baseUri("/auth/authorize")//기본값이 아닌 /auth/authorize/github형태로 요청
         .and()
         .userInfoEndpoint() //OAuth2 인증이 성공한 후, 사용자 프로필 데이터를 가져오는 역할
         .userService(oAuthUserService)//사용자 정보를 처리하는 서비스를 지정하는 메서드
         .and()
   ...
   public CorsConfigurationSource corsConfigurationSource() {
      ...
      configuration.setAllowedOrigins(
            Arrays.asList(
                  "http://localhost:3000",
                  "http://app.springbootlhj.store/",
                  "https://app.springbootlhj.store/"
      ));
      //허용할 메서드 지정
